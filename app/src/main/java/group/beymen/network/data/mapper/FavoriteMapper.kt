...
fun Product.toFavoriteProductEntity(): FavoriteProductEntity {
    return FavoriteProductEntity(
        ...
        imageUrls = this.MediaList?.map { it.ResizedUrlPath } ?: emptyList(),
...
fun FavoriteProductEntity.toProduct(): Product {
    return Product(
        ...
        FirstProductImageURL = this.imageUrls.firstOrNull() ?: "",
        ID = this.id,
        IsHero = false,
        ...
        LabelPrice = this.labelPrice,
        MediaList = this.imageUrls.map { Media(ID = 0, DisplayOrder = 0, ResizedUrlPath = it) },
        ProductType = "",
